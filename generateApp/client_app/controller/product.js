               ...
               product_module.product_count_page_filter(filterObj,limit_page,function (err,result3) {
                    ...
                    if (Array.isArray(result3) == true && result3.length)
                    {
                         ...
                         product_module.product_get_all_limit_filter(filterObj,current_page*limit_page,limit_page,'table_pk','DESC',function (err,result4) {
                              ...
                              if(req.baseUrl == "/api") {
                              res.send({products : result4, brands : result1, sub_category : result2, urlQuery: urlQuery, info:{count_page : count_page, limit_page:limit_page, current_page : parseInt(current_page)+1}, err : err, session : req.session});
                                   } else {
                              res.render('products_list',{products : result4, brands : result1, sub_category : result2, urlQuery: urlQuery, info:{count_page : count_page, limit_page:limit_page, current_page : parseInt(current_page)+1}, err : err, session : req.session});
                              }        
                         ...
                         if(req.baseUrl == "/api") {
                    res.send({products:[], brands : result1, sub_category : result2, urlQuery: urlQuery, info:{count_page : 0, limit_page:limit_page, current_page : parseInt(current_page)+1}, err : err, session : req.session})
                         } else {
                    res.render('products_list',{products : [], brands : result1, sub_category : result2, urlQuery: urlQuery, info:{count_page : count_page, limit_page:limit_page, current_page : parseInt(current_page)+1}, err : err, session : req.session});
                         }
                    }
               })
